...
<script type="text/ecmascript-6">
  ...
  export default {
      ...
      getDeliveryItem(params).then((res) => {
        ...
        if (res.data.code === 200) {
          ...
          for (let i = 0; i < data.itemList.length; i++) {
            ...
            data.itemList[i].realQuantity = parseFloat(data.itemList[i].noticeQuantity, 10) - parseFloat(data.itemList[i].historyQuantity, 10)
            ...
            data.itemList[i].realQuantity = Number(data.itemList[i].realQuantity).toFixed(3)
          }
          // 合计
          this.itemList.push({
            number: '合计',
    ...
    methods: {
      maxSend(val) {
        if (parseFloat(val.row.noticeQuantity, 10) - parseFloat(val.row.historyQuantity, 10) < parseFloat(val.row.realQuantity, 10)) {
          this.itemList[val.$index].realQuantity = parseFloat(val.row.noticeQuantity, 10) - parseFloat(val.row.historyQuantity, 10)
        }
        let total = 0
        for (let i = 0; i < this.itemList.length - 1; i++) {
          total += parseFloat(this.itemList[i].realQuantity, 10)
        }
