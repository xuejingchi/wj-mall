      ...
      <div class="content" v-for="items in orderList">
        ...
        <ul class="body">
          ...
          <li class="label-6">
            ...
            <button @click="goDeliveryManagement(items.orderCode)" v-if="items.status === '60' && canGoDelivery">我要发货</button>
          </li>
          <li class="label-7">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created () {
      let menuList = getUserInfo().menuList.supplier.menuList
      let arr = menuList.filter(function (item) {
        return item.url === 'setting/supplier/deliveryManagement'
      })
      if (arr.length) {
        this.canGoDelivery = true
      } else {
        this.canGoDelivery = false
      }
    },
    beforeMount () {
    },
