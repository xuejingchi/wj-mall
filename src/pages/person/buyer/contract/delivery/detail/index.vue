  ...
  <section class="delivry-detail-page" v-loading.fullscreen.lock="loading">
    ...
    <div class="delivry-detail-main">
        ...
        <div class="info">
          ...
          <div class="bottom">
            ...
            <div>
              ...
              <ul style="float: left;" class="mr-l delivery-logist-list" v-if="info.logisticsList">
            ...
            <el-table
              ...
              <template slot-scope="scope">
                <ul class="commodity-info">
                  <li>{{scope.row.itemName}}</li>
                  <li>品牌：<span>{{scope.row.brandName}}</span></li>
                  <li>技术参数：<span>{{scope.row.paramters}}</span></li>
                </ul>
              </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template slot-scope="scope">
                  <!-- 如下情况禁用：1、详情页无法操作；2、状态不为到货待签收 -->
                  <input :disabled="!btnShow || info.status !== '50'" type="number" v-model="scope.row.receiptQuantity" @change="receiptQuantityChange(scope)"/>
                </template>
              </el-table-column>
            </el-table>
          </template>
      ...
      <div v-if="btnShow && info.status === '50'" class="detail">
        <div class="info">
          <div class="top"><span style="color: #FD2E2E; margin-right: 2px;">*</span>签收凭证</div>
          <div class="bottom">
      ...
      <div v-if="btnShow && info.status === '50'" class="detail">
        <div class="info">
          <div class="top"><span style="color: #FD2E2E; margin-right: 2px;">*</span>签收说明：</div>
          <div class="bottom">
        ...
        <div class="info">
          ...
          <div class="bottom">
            ...
            <p style="color: #1c6eeb;" class="file-uploaded-name" @click="DownloadFile(info.attachmentPath, info.attachmentName)">
              <i style="margin-right: 5px;" class="iconfont icon-fujian" v-if="info.attachmentName"></i>{{ info.attachmentName }}
            </p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      * 实发量后三位四舍五入
      * */
      receiptQuantityChange (scope) {
        console.log('实发--->', scope.row.realQuantity)
        console.log('签收--->', scope.row.receiptQuantity)
        console.log(scope.row)
        scope.row.realQuantity = Number(scope.row.realQuantity).toFixed(3)
        scope.row.receiptQuantity = Number(scope.row.receiptQuantity)
        if (scope.row.receiptQuantity > scope.row.realQuantity) {
          console.log(1111)
        } else {
          console.log(2222)
        }
        if (scope.row.receiptQuantity > scope.row.realQuantity) {
          scope.row.receiptQuantity = scope.row.realQuantity
        }
        scope.row.receiptQuantity = Number(scope.row.receiptQuantity).toFixed(3)
      },
      /*
      * 加载数据
      * */
      loadData () {
        getDeliveryInfo({
          ...
          info.itemList.forEach((item, index) => {
            ...
            // 签收量默认等于实发量
            if (info.status !== '90') {
              item.receiptQuantity = Number(item.realQuantity).toFixed(3)
            } else {
              item.receiptQuantity = Number(item.receiptQuantity).toFixed(3)
            }
            item.noticeQuantity = Number(item.noticeQuantity).toFixed(3)
            item.realQuantity = Number(item.realQuantity).toFixed(3)
          })
      ...
      * */
      getBillingSummaries (param) {
        return getSummaries(param, ['contractTotal', 'noticeQuantity', 'realQuantity', 'receiptQuantity'])
      }
    }
  }
</script>
