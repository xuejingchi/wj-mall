<template>
  <div class="subaccount">
    ...
    <div class="list">
      ...
      <div v-loading="loading">
        ...
        <div class="table-list-body " v-for="(table, index) in tableItems2" @mouseenter="table.show = true" @mouseleave="table.show = false" >
          <div class="table-body-inner" :class="{'col-F7':table.show, 'col-f2':table.status === '3'}" >{{table.accoutName}}</div>
          <div class="table-body-inner" :class="{'col-F7':table.show, 'col-f2':table.status === '3'}" @click="ModifySubaccount(table.accountId)">
            <span v-if="table.show && table.status !== '3'" style="color:#1C6EEB;">{{table.roleName}}<i class="el-icon-edit-outline"></i></span>
            <span v-else>{{table.roleName}}</span>
          </div>
          <div class="table-body-inner" :class="{'col-F7':table.show, 'col-f2':table.status === '3'}">
            {{table.phoneNumber}}
          </div>
          <div class="table-body-inner" :class="{'col-F7':table.show, 'col-f2':table.status === '3'}">
          ...
          <div class="table-body-inner" :class="{'col-F7':table.show, 'col-f2':table.status === '3'}">
            <span v-if="table.status === '0'">
              <el-button type="text" @click="cancel(table)" size="mini">注销</el-button>
            </span>
        ...
        <div style="border-bottom:1px solid #E2E2E2">
          <div style="text-align:center;padding-top:50px;padding-bottom:30px">
            ...
            <div class="goBuy" style="height:38px;line-height:38px" @click="">
              <el-button size="medium" style="width:150px;margin-bottom:20px;" @click="goCreate">去创建</el-button></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <el-dialog
      ...
      width="30%">
      <span>确认注销此账户？此账户一旦注销，将无法恢复，请知悉。</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="confimCancel">确 定</el-button>
      </span>
    </el-dialog>
      </div>
</template>
...
export default {
  ...
  methods: {
    ...
    cancel (item) {
      ...
      this.cancelAccountId = item.accountId
    },
    // 确定注销
    confimCancel () {
      let params = {
        accountId: this.cancelAccountId,
        parentAccountId: this.userId,
        parentAccoutName: getUserInfo().userName,
        status: 3
      }
