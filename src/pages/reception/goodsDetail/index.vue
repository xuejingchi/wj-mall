<template>
  <div class="goods-detail-main" v-loading.fullscreen.lock="loading">
    ...
    <section class="goods-detail-info">
        ...
        <div class="detail-info-previews">
          ...
          <div class="swiper-container">
            <div class="swiper-wrapper" :style="swiperStyleObj">
              <div class="swiper-slide" v-if="itemInfo.imageList && itemInfo.imageList.length" :class="{ active: index === imgShowIndex }" v-for="(item, index) in itemInfo.imageList" @click="changeImgShow(item, index)" :style="{ backgroundImage: 'url(' + item.picExFileid +  '?otype=max&w=60&h=60'+')' }">
                <!--<img :src="item">-->
                <!--<span>{{ item }}</span>-->
              </div>
            </div>
      ...
      <div class="detail-info-main">
        ...
        <!-- 提示内容 -->
        <ul class="detail-info-des" v-if="tipList.length">
        ...
        <section class="detail-info-footer">
          ...
          <div class="detail-service-box">
            ...
            <span class="detail-service-num">400-826-7880</span>
          </div>
        </section>
      </div>
    </section>
    ...
    <!-- 商品详情 -->
    <section v-if="detailData" class="detail-info-detail">
      <nav class="detail-nav">
        <p class="nav-left-title">详细描述</p>
        <div class="nav-right-block"></div>
      </nav>
      <div v-if="detailData" class="info-detail-desc">
        <!--{{ detailData }}-->
        <div v-if="detailData.itemDescDTOList && detailData.itemDescDTOList.length">
        ...
        <div v-if="detailData.imageDTOList && detailData.imageDTOList.length" style="margin-top: 20px;">
          <div style="margin-bottom: 10px; text-align: center;" v-for="item in detailData.imageDTOList">
            <img width="790px" v-if="item.picExFileid" :src="`${ServerImgUrl}${item.picExFileid}?otype=max&w=790`" />
          </div>
        </div>
      </div>
    </section>
...
export default {
  ...
  computed: {
    ...
    * */
    activeImgUrl () {
      ...
      return `${this.itemInfo.imageList[this.imgShowIndex].picExFileid}?otype=max&w=400&h=400`
    },
    /*
    * 图片滑动样式
    * */
  ...
  methods: {
    ...
    * */
    configParamEnable (attributes) {
      for (let i = 0; i < attributes.length; i++) {
        ...
        let name = this.getSelectedName(attributes[i])
        if (values && values.length) {
          this.disableOtherParam(values, i, attributes, name)
        }
      }
    },
    /*
    ...
    * 使其他的不可选
    * */
    disableOtherParam (configValues, configIndex, attributes, configName) {
      // 遍历原数组
        ...
        if (configIndex !== index) {
          attribute.values.forEach(value => {
            if (!this.getIntersection(value.values, configValues).length && value.enable) {
              ...
              console.log('name: ', value.name, 'name2: ', configName)
              value.enable = false
            }
            // 前端不需要判断是否选中状态
