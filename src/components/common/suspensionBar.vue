<template>
  <div id="suspension" v-show="showSuspension">
    <ul id="suspension-list">
      <li class="consult" @click="isActive(0)">
        <i class="iconfont">&#xe626;</i>
        <p class="text">咨询</p>
        <p class="text">客服</p>
        <div class="service">
      ...
      <li class="public-number" @click="isActive(3)">
        <i class="iconfont">&#xe638;</i>
        <p class="text">公</p>
        <p class="text">众</p>
        <p class="text">号</p>
        <div class="public-content">
          <span class="iconfont close-icon" @click="closeFloat()">&#xe605;</span>
          <div class="public-des">关注万郡绿建交易平台</div>
          <img src="http://mall.vandream.com:8081/group1/M01/01/21/rBCwJFuGCxmABvRFAADil7CiQSQ177.png" alt="" >
        </div>
      </li>
...
export default {
  ...
  methods: {
    ...
    isActive (num) {
      ...
      let li = ul.children
      if (li[num].className.indexOf('active') === -1) {
        li[num].className += ' active'
      } else {
        let item = li[num].className
        let newItem = item.replace('active', '')
        li[num].className = newItem.replace(/(^\s*)|(\s*$)/g, '')
      }
    }
  },
  created () {
